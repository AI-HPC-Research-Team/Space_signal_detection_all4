...
class GW_SE_Dataset(object):
    def __init__(self, length=16000, sample_rate=0.1, use_AAK=False, use_NRSur=False, use_PyCBC=False, BWD=False, SGWB=False):
        #parameter range
        if use_AAK:
        ...
        if BWD:
            ...
            self.param_idx = param_idx
            self.nparams = nparams
            self.BWD_par_range = dict(f1=[1e-3, 4e-3], f_dot1=[-4e-17, 7e-16], f2=[4e-3, 15e-3], f_dot2=[-3e-15, 4e-14])
    ...
    def gen_bbh_signal_nrsur(self, m1, m2, f_low=1e-5, chiA=[0, 0, 0], chiB=[0, 0, 0]):
        ...
        t, h, dyn = self.NRSur(
            q, chiA, chiB, dt=self.delta_t, f_low=f_low, mode_list=[(2, 2)], M=M, dist_mpc=dist_mpc, units='mks'
        )
    ...
    def get_inner_product(self, data1, data2):
        ...
        SNRsq = 2.0 * np.sum(
            (xf1[fidx:] * np.conjugate(xf2[fidx:]) + np.conjugate(xf1[fidx:]) * xf2[fidx:]) * invpsd[fidx:]
        ) * df
        return SNRsq.real
    ...
    def generate_emri_dataset(self, n_grid=[4, 3]):
        ...
        for pkey, idx in self.param_idx.items():
            ...
            par[idx, :] = np.linspace(
                self.AAK_par_range[pkey][0] + dpd2, self.AAK_par_range[pkey][1] - dpd2, num=grid_per_par, endpoint=True
            )
    ...
    def generate_bwd_dataset(self, n_grid=[4, 3, 11, 7]):
        ...
        par1[0, :] = np.linspace(self.BWD_par_range['f1'][0], self.BWD_par_range['f1'][1], num=n_grid[0], endpoint=True)
        par1[1, :] = np.linspace(self.BWD_par_range['f_dot1'][0], self.BWD_par_range['f_dot1'][1], num=n_grid[0], endpoint=True)
        par2[0, :] = np.linspace(self.BWD_par_range['f2'][0], self.BWD_par_range['f2'][1], num=n_grid[2], endpoint=True)
        par2[1, :] = np.linspace(self.BWD_par_range['f_dot2'][0], self.BWD_par_range['f_dot2'][1], num=n_grid[2], endpoint=True)
        ...
        par1[0, :] = np.linspace(
            self.BWD_par_range['f1'][0] + dpd2, self.BWD_par_range['f1'][1] - dpd2, num=n_grid[1], endpoint=True
        )

        dpd2 = (self.BWD_par_range['f_dot1'][1] - self.BWD_par_range['f_dot1'][0]) / (par1.shape[1]) / 2
        par1[1, :] = np.linspace(
            self.BWD_par_range['f_dot1'][0] + dpd2, self.BWD_par_range['f_dot1'][1] - dpd2, num=n_grid[1], endpoint=True
        )

        dpd2 = (self.BWD_par_range['f2'][1] - self.BWD_par_range['f2'][0]) / (par1.shape[1]) / 2
        par2[0, :] = np.linspace(
            self.BWD_par_range['f2'][0] + dpd2, self.BWD_par_range['f2'][1] - dpd2, num=n_grid[3], endpoint=True
        )

        dpd2 = (self.BWD_par_range['f_dot2'][1] - self.BWD_par_range['f_dot2'][0]) / (par1.shape[1]) / 2
        par2[1, :] = np.linspace(
            self.BWD_par_range['f_dot2'][0] + dpd2, self.BWD_par_range['f_dot2'][1] - dpd2, num=n_grid[3], endpoint=True
        )
    ...
    def generate_smbhb_dataset(self, n_grid=[4, 3]):
        ...
        par = np.zeros([self.nparams, grid_per_par])
        for pkey, idx in self.param_idx.items():
            par[idx, :] = np.linspace(self.bbh_par_range[pkey][0], self.bbh_par_range[pkey][1], num=grid_per_par, endpoint=True)
        ...
        for pkey, idx in self.param_idx.items():
            ...
            par[idx, :] = np.linspace(
                self.bbh_par_range[pkey][0] + dpd2, self.bbh_par_range[pkey][1] - dpd2, num=grid_per_par, endpoint=True
            )
    ...
    def save_waveform(self, DIR='.', data_fn='waveform_dataset.hdf5'):
        ...
        for i in self.waveform_par.keys():
            data_name = i + '_' 'par'
            f_data.create_dataset(data_name, data=self.waveform_par[i], compression='gzip', compression_opts=9)
        f_data.close()
